  ...
  function renderRows() {
    ...
    tbody.innerHTML = currentMembers.map((m, i) => `
      <tr data-searchable="${(m.name || '').toLowerCase()} ${(m.rank || '').toLowerCase()}">
        ...
        <td style="font-weight:500;"><a href="player.html?view=war&id=${encodeURIComponent(m.name||'')}" class="member-link">${m.name || '—'}</a></td>
    ...
    tbody.innerHTML = currentPlayers.map((p, i) => {
      ...
      return `
        <tr data-searchable="${(p.name || '').toLowerCase()}">
          <td class="mono" style="color:var(--text-muted);">${i + 1}</td>
          <td style="font-weight:500;"><a href="player.html?view=hunt&id=${encodeURIComponent(p.name||'')}" class="member-link">${p.name || '—'}</a></td>
